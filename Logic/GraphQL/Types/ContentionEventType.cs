using CoTEC_Server.DBModels;
using HotChocolate.Types;

namespace CoTEC_Server.Logic.GraphQL.Types
{
    public class ContentionEventType : ObjectType<ContentionMeasuresChanges>
    {

        protected override void Configure(IObjectTypeDescriptor<ContentionMeasuresChanges> descriptor)
        {
            base.Configure(descriptor);

            descriptor.BindFieldsExplicitly();

            descriptor.Name("ContentionEvent");

            descriptor.Field(f => f.MeasureNameNavigation)
                .Type<NonNullType<ContentionMeasureType>>()
                .Name("measure");

            descriptor.Field(f => f.StartDate)
                .Type<NonNullType<DateType>>();

            descriptor.Field(f => f.EndDate)
                .Type<DateType>();

            descriptor.Field(f => f.CountryNameNavigation)
                .Type<NonNullType<CountryType>>()
                .Name("country");
        }

    }
}
